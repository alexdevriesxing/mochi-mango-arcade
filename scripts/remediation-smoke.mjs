...
const targets = [
  'pixel-panda-parkour-standalone',
  'pixel-prawn-deep-sea-debugger-standalone',
  'boom-bap-cannon',
  'super-seans-merge-madness',
  'super-seans-pipe-puzzle',
  'puddle-pip-meadow-dash',
  'puddles-pancake-panic',
  'mushmoos-moonlit-match',
  'bloop-bubble-rescue',
  'nine-gates-mahjong-trails',
  'snackstreet-rush',
  'crownlight-chess',
  'starling-signal-patrol',
  'glitch-garden',
  'boot-sector',
  'rooftop-rocket-rumble',
  'tower-of-floppy',
  'super-seans-racing-rally',
  'super-seans-soccer-showdown',
  'super-seans-pinball-party',
  'comet-quarry-crew',
  'floppy-flap',
  'baos-jade-dragon-rescue',
  ...
  for (const slug of targets) {
    ...
    try {
      ...
      const metrics = await page.evaluate(() => ({
        canvas: [...document.querySelectorAll('canvas')].some(node => {
          const r = node.getBoundingClientRect(); return r.width > 120 && r.height > 120;
        }),
        iframe: [...document.querySelectorAll('iframe')].some(node => {
          const r = node.getBoundingClientRect(); return r.width > 120 && r.height > 120;
        }),
      ...
      const fatal = errors.filter(error => !/cloudflareinsights|favicon|404.*favicon/i.test(error));
...
await browser.close();
console.log(JSON.stringify({ tested: results.length, passed: results.filter(r => r.ok).length, failures }, null, 2));
