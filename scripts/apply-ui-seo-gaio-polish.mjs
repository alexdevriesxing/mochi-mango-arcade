...
const replace=(text,needle,value,label)=>{
  if(!text.includes(needle))throw new Error(`Missing replacement target: ${label}`);
  return text.replace(needle,value);
};

let app=fs.readFileSync(appPath,'utf8');
app=replace(app,'<a class="skip-link" href="#main">Skip</a>','<a class="skip-link" href="#main">Skip to main content</a>','skip link');
app=replace(app,'<h1>200+<br><span>Playful HTML5 Games</span></h1>','<h1>${built.length} free games.<br><span>One playful universe.</span></h1>','home hero count');
app=replace(app,'<p>${built.length} playable today with fresh games and collectible merch added every week.</p>','<p>Instant browser games with original characters, campaign goals and daily reasons to come back — no download and no account required.</p>','home hero copy');
app=app.replace(/slice\(0,5\)/g,'slice(0,4)');
app=replace(app,'<p>Browse all 200 games — <b>${S.games.filter(g=>g.built).length} playable now</b>, with more added every week.</p>','<p>Browse all <b>${S.games.length} free games</b> — every title is playable now on mobile and desktop.</p>','catalog count');
app=replace(app,'<p>Mochi Mango Arcade is a scalable HTML5 game and merch platform by Fire Dragon Interactive for 200+ games, recurring universes, ad monetization and collectibles.</p>','<p>Mochi Mango Arcade is a family-friendly browser arcade by Fire Dragon Interactive, built around original games, recurring characters and transparent free-to-play access.</p>','about hero');
...
for(const file of walk(publicDir).filter(file=>file.endsWith('.html'))){
  ...
  if(!html.includes('name="application-name"'))html=html.replace('<meta name="theme-color" content="#ff4f9a">','<meta name="theme-color" content="#6a32d5"><meta name="application-name" content="Mochi Mango Arcade"><meta name="color-scheme" content="light"><meta name="referrer" content="strict-origin-when-cross-origin">');
  ...
  if(game){
    ...
    html=html.replace(/<meta property="og:image" content="[^"]+">/,`<meta property="og:image" content="${og}"><meta property="og:image:type" content="image/jpeg">`);
    html=html.replace(/<meta name="twitter:image" content="[^"]+">/,`<meta name="twitter:image" content="${og}"><meta name="twitter:image:alt" content="${String(game.title).replaceAll('"','&quot;')} — free browser game">`);
    html=html.replace(/<script type="application\/ld\+json">(.*?)<\/script>/s,(full,jsonText)=>{
      try{
        ...
        const node=graph.find(item=>item['@type']==='VideoGame');
        if(node){
...
const catalogue={
  name:'Mochi Mango Arcade game catalogue',
  canonicalUrl:'https://www.mochimangoarcade.com/',
  publisher:{name:'Fire Dragon Interactive',url:'https://www.firedragoninteractive.com'},
  lastVerified:today,
  gameCount:games.length,
  pricing:'Free to play',
  accountRequired:false,
  games:games.map(game=>({
    id:game.slug,title:game.title,url:`https://www.mochimangoarcade.com/games/${game.slug}/`,playUrl:`https://www.mochimangoarcade.com/play/${game.slug}/`,genre:game.genre,character:game.mascot,universe:game.universeName||universes[game.universe]?.name||game.universe,description:game.description,objective:game.objective,version:game.version||'1.0',releaseDate:game.releaseDate||today,image:`https://www.mochimangoarcade.com${game.image}`
  }))
};
...
const entityGraph={
  ...
  '@graph':[
    {'@type':'Organization','@id':'https://www.firedragoninteractive.com/#organization',name:'Fire Dragon Interactive',url:'https://www.firedragoninteractive.com'},
    {'@type':'WebSite','@id':'https://www.mochimangoarcade.com/#website',name:'Mochi Mango Arcade',url:'https://www.mochimangoarcade.com/',publisher:{'@id':'https://www.firedragoninteractive.com/#organization'},inLanguage:'en',isAccessibleForFree:true},
    {'@type':'CollectionPage','@id':'https://www.mochimangoarcade.com/games/#collection',name:'Mochi Mango Arcade Games',url:'https://www.mochimangoarcade.com/games/',numberOfItems:games.length,about:Object.entries(universes).map(([id,u])=>({'@type':'CreativeWorkSeries','@id':`https://www.mochimangoarcade.com/universes/#${id}`,name:u.name,description:u.description}))},
    ...Object.entries(characterCounts).map(([name,count])=>({'@type':'FictionalCharacter',name,url:`https://www.mochimangoarcade.com/characters/#${name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}`,description:`Original Mochi Mango Arcade character appearing in ${count} games.`}))
  ]
};
